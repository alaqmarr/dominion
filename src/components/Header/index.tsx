...
const Header = () => {
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        const response = await fetch("https://mystore.alaqmar.dev/api/dominion/categories", {
                    ...
                    {menuData.map((menuItem, index) =>
                      menuItem.path ? (
                        ...
                        <li className="submenu-item group relative" key={index}>
                          ...
                          >
                            {categories.map((submenuItem: any, i) => (
                              <Link
                                href={`/category/${submenuItem.id}`}
                                key={i}
                                className={`block rounded px-4 py-[10px] text-sm ${
                                  pathUrl === submenuItem.id
                                    ? "text-primary"
